...
function getNextFriday(date) {
  ...
  const daysUntilNextFriday = ((11 - date.getDay()) % 7) + 1;
...
function getWeekNumberByDate(date) {
  ...
  const daysOfYear = (date - startOfYear) / (24 * 60 * 60 * 1000) + 1;
  return Math.floor((daysOfYear + startOfYear.getDay() + 6) / 7);
}
...
function getNextFridayThe13th(date) {
  let nextFriday13Date = new Date(date.getFullYear(), date.getMonth(), 13);
  while (nextFriday13Date.getDay() !== 5) {
    if (nextFriday13Date.getMonth() === 11) {
      nextFriday13Date = new Date(nextFriday13Date.getFullYear() + 1, 0, 13);
    } else {
      nextFriday13Date = new Date(
        nextFriday13Date.getFullYear(),
        nextFriday13Date.getMonth() + 1,
        13
      );
    }
  }
  return nextFriday13Date;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const currentDay = periodOfDate(period.start);
  const end = periodOfDate(period.end);
  const schedule = [];

  while (currentDay <= end) {
    for (let i = 0; i < countWorkDays; i += 1) {
      if (currentDay <= end) {
        schedule.push(dateToSchedule(currentDay));
